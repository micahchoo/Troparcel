...
rounded integer coordinates:

```
SHA-256("sel:{photoChecksum}:{round(x)}:{round(y)}:{round(w)}:{round(h)}")
  → 24-char hex key
```

### Note Key

Notes are identified by content hash and parent association:

```
SHA-256("note:{photoChecksum}:{first 200 chars of html or text}")
  → 24-char hex key
```

The 200-character truncation means notes with identical openings but
different endings will collide — this is by design to enable stable
identity across edits to the same note.
...
```
SHA-256("tx:{photoChecksum}:{index}") → 24-char hex key
SHA-256("tx:{photoChecksum}:{selKey}:{index}") → for selection transcriptions
```
...
| Re-tagging | Adding a tag with `deleted: true` clears the tombstone; no author check on re-add |
...
| Apply attribution | Remote notes are prefixed with `[author]` in bold |
...
| Aspect | Detail |
|--------|--------|
| CRDT type | `Y.Map` keyed by `SHA-256(sel:{checksum}:{x}:{y}:{w}:{h})` |
...
| Aspect | Detail |
|--------|--------|
| CRDT type | `Y.Map` keyed by `SHA-256(tx:{checksum}:{idx})` |
...
and lists are tombstoned, the item is blocked. This protects against
bulk deletion from a rogue peer.
...
- **Stripped:** `<script>`, `<style>`, `<iframe>`, `<embed>`, `<object>`,
  `<form>`, `<input>`, `<textarea>`, `<button>`, `<select>` and their content
- **Blocked attributes:** `on*` event handlers, `data-*`, `style`
- **Blocked protocols:** `javascript:`, `vbscript:`, `data:` (including
  entity-encoded variants like `&#x6A;avascript:`)
- **Allowed:** `<p>`, `<em>`, `<strong>`, `<a href>`, `<br>`, `<ul>`,
  `<ol>`, `<li>`, `<blockquote>`, `<h1>`–`<h6>`, `<sup>`, `<sub>`,
  `<s>`, `<u>`, `<code>`, `<pre>`, `<span>`, `<div>`, `<table>`,
  `<tr>`, `<td>`, `<th>`, `<thead>`, `<tbody>`
- **Allowed attributes:** `href` (on `<a>`, validated), `class` (global)
...
4. Re-fetches and re-saves notes and selections
5. Returns `{ restored: count, errors: [...] }`

Rollback uses the HTTP API (not store dispatch) and works even if the
...
|--------|---------|----------|
| `syncMetadata` | `true` | Item and selection metadata |
...
### Note Content Key Truncation

Note keys use only the first 200 characters of content. Two notes that
share the same opening 200 characters but differ afterward will produce
the same key. This is intentional (enables stable identity across edits)
but can cause unintended merging of distinct notes with similar openings.
...
Tombstones are never garbage-collected from the CRDT. Over time, a
heavily-edited project will accumulate tombstones that increase document
size and slow down snapshot serialization. The tombstone flood threshold
only protects against sudden spikes, not gradual accumulation.
...
| `_waitForAction()` timeout | Prevents indefinite waits for Redux saga completion | 15s timeout with resolve-anyway |
